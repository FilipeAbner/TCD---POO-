 ...
 * @author Natã
 */
public class ExcluirGenero extends javax.swing.JPanel {
    ...
    private void pnlbtnCadastrar1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_pnlbtnCadastrar1MouseClicked
        Livro livroLocalizado =
            new LivroDao().localizarLivrosPorGenero(generoAtributo.getId());
        
        if(livroLocalizado != null){
            
            JOptionPane.showMessageDialog(
                null, "Não é possível excluir um gênero que possui "
        ...
        new GeneroDao().deletarDeFato(generoAtributo);

        JOptionPane.showMessageDialog(
            null, "Gênero exluido com sucesso.");
